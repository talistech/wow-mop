-- Battle pet tamers can have per-spawn npcflag/script overrides in `creature`.
-- Keep the placed NPCs aligned with their templates so quest clicks open the
-- tamer gossip script instead of falling through to static text/no options.
